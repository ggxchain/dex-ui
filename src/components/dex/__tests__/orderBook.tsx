...
describe("OrderBook", () => {
    const token1 = { id: 1, symbol: "ETH", name: "Ethereum", network: "ETH", decimals: 18 };
    const token2 = { id: 2, symbol: "BTC", name: "Bitcoin", network: "BTC", decimals: 8 };
    ...
    test("renders with orders", () => {
        render(<OrderBook sellToken={token1} buyToken={token2} onChange={jest.fn()} orders={orders} />);
        expect(screen.getByText("Order book")).toBeInTheDocument();
    ...
    test("calls onChange when a buy order is not selected", () => {
        ...
        render(<OrderBook sellToken={token1} buyToken={token2} onChange={onChange} orders={orders} />);
    ...
    test("on change shouldn't be called if orders is reverse", () => {
        ...
        render(<OrderBook sellToken={token1} buyToken={token2} onChange={onChange} orders={ordersTmp} />);
