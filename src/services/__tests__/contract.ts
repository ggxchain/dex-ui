...
describe('Contract', () => {
    ...
    it('should fail to withdraw if amount is <= 0', async () => {
        await expect(contract.withdraw(mockedTokenId, BN_ZERO, () => { })).rejects.toThrow(Errors.AmountIsLessOrEqualToZero)
        await expect(contract.withdraw(mockedTokenId, BN_ONE.neg(), () => { })).rejects.toThrow(Errors.AmountIsLessOrEqualToZero)

        // Success case
        // We need to deposit first
        await expect(contract.deposit(mockedTokenId, BN_ONE, () => { })).resolves.not.toThrow()
        await expect(contract.withdraw(mockedTokenId, BN_ONE, () => { })).resolves.not.toThrow()
    });

    it('should fail to withdraw if not enough balance', async () => {
        await expect(contract.withdraw(mockedTokenId, BN_ONE, () => { })).rejects.toThrow(Errors.NotEnoughBalance)
