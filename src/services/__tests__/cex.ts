...
describe('CexService', () => {
    ...
    const mock = () => {
        ...
        window.localStorage.setItem('prices.lastFetchedArray', JSON.stringify(['btc', 'usdt']));
    }
    ...
    it('should fetch new prices for tokens not in cache', async () => {
        global.fetch = jest.fn().mockResolvedValue({ json: () => ({ BTC: { USD: 10 }, USDT: { USD: 20 }, TOKEN3: { USD: 30 }, TOKEN4: { USD: 40 } }) });
        ...
        const prices = await cexService.tokenPrices(['btc', 'usdt', 'token3', 'token4']);
    ...
    it('should fetch new prices for tokens if experied', async () => {
        mock();
        const cexService = new CexService();
        global.fetch = jest.fn().mockResolvedValue({ json: () => ({ BTC: { USD: 10 }, USDT: { USD: 20 } }) });
