...
export default class GGXWallet {
    ...
    constructor() {
        ...
        const selectedAccount = window.localStorage.getItem("ggx-wallet-selected-account");
    ...
    async getAccounts(): Promise<Account[]> {
        ...
        window.localStorage.setItem("ggx-wallet-accounts", JSON.stringify(this.accounts));
        if (this.accounts.length > 0 && !this.selectedAccount) {
            this.selectedAccount = this.accounts[0];
            window.localStorage.setItem("ggx-wallet-selected-account", JSON.stringify(this.accounts[0]));
        }
    ...
    async selectAccount(account: Account): Promise<void> {
        ...
        window.localStorage.setItem("ggx-wallet-selected-account", JSON.stringify(account));
