...
export function ibcHashToDenom(chainName: string, ibcHash: string) {
	...
	const chainConnections = ibc.filter(
		(info) =>
	...
	for (const connection of chainConnections) {
		...
		const otherChannels = connection.channels.map((channel) =>
			isFirstChain ? channel.chain_2 : channel.chain_1,
		);
		const otherChainName = otherChain.chain_name;
		const otherChainAssets = assets.find(
			(asset) => asset.chain_name === otherChainName,
		)?.assets;
		if (!otherChainAssets) {
			continue;
		}
		...
		for (const channel of otherChannels) {
			for (const asset of otherChainAssets) {
				...
				const testedHash = createHash("sha256")
					.update(path)
					.digest("hex")
					.toUpperCase();
